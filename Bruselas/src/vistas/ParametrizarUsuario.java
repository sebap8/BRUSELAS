...
public class ParametrizarUsuario extends JFrame {
	...
	public ParametrizarUsuario() {
		...
		setTitle("PARAMETRIZAR USUARIOS");
		...
		Collections.sort(a, EmpleadoBean.ComparoFechaComparator);
		for(int i=0;i<a.size();i++){
			comboBox.addItem(a.get(i).getNombre());
		}
		...
		btnAceptare.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(Controlador.getInstancia().cambiarPermisos(permisos,comboBox.getSelectedItem().toString())){
					...
					JOptionPane.showMessageDialog(null, "No se han podido otorgar los permisos. Comuníquese con el administrador.","ERROR", JOptionPane.ERROR_MESSAGE);
				}
			}
		});
